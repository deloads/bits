...
lBits = read_bits_from_file("bitc.bm")
eBits = []
for i in range(256):
    eBits.append(lBits[256*i:256*(i+1)])

def encript(bits,key):
    for i in range(len(key)):
        currentSet = eBits[ord(key[i])]
...
def decript(bits,key):
    for i in range(len(key)-1,-1,-1):
        currentSet = eBits[ord(key[i])]
        for j in range(len(bits)-1,-1,-1):
            n = currentSet.index(bits[j])-j
            while n<0:
                n+=255
            bits[j] = n

def readData(path,key = False):
    bits = read_bits_from_file(path)
    if key:
        decript(bits,key)
    try:
        data = convertbits(bits)
    except:
        data = "invalid key!"
    return data

def storeData(data,path,key = False):
    bits = convertData(data)
    if key:
        encript(bits,key)
    save_bits_to_file(path,bits)
